...
//Notification Setup using BullMQ

const {Queue} = require("bullmq");
const notificationQueue = new Queue('email_queue');

async function addToQueue(title, blogId, email) {
  text = 'Congratulations! Your blog '+ title + 'has been successfully published! You can view it at http://localhost:8000/blog/'+blogId
  const result = await notificationQueue.add('email user',{
    'from': 'iit2021223@iiita.ac.in',
    'to': email,
...
router.post("/", upload.single('coverImage'), async (req, res) => {
    ...
    //send email here about blog getting published
    try {
        addToQueue(title, blog._id, req.user.email);
    }
    catch(err){
